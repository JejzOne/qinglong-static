...
async function getNetIp(req) {
    ...
    function isLan(ip) {
        ...
        if (ip.startsWith('192.168.'))
            return true;
        if (ipNumArray[0] === '172' &&
            Number(ipNumArray[1]) >= 16 &&
            Number(ipNumArray[1]) <= 31) {
            return true;
        }
        return false;
    }
    ...
    for (let i = 0; i < ipArray.length; i++) {
        const ip = cleanIp(ipArray[i]);
        if (!ip)
            continue;
        if (!isLan(ip) && !publicIp) {
            publicIp = ip;
        }
        if (isLan(ip) && !lanIp) {
            lanIp = ip;
    ...
    let finalIp = publicIp || lanIp || '';
    if (!finalIp) {
        return { address: `获取失败`, ip: '' };
    }
    // 仅公网IP做地理查询，内网直接返回
    if (isLan(finalIp)) {
        return { address: `内网IP`, ip: finalIp };
    }
    try {
        const baiduApi = got_1.default
            .get(`https://www.cip.cc/${finalIp}`, { timeout: 10000, retry: 0 })
            .text();
        const ipApi = got_1.default
            .get(`https://whois.pconline.com.cn/ipJson.jsp?ip=${finalIp}&json=true`, {
            timeout: 10000,
            retry: 0,
        })
            .buffer();
        const [data, ipApiBody] = await Promise.all([baiduApi, ipApi]);
        const ipRegx = /.*IP	:(.*)\n/;
        const addrRegx = /.*数据二	:(.*)\n/;
        if (data && ipRegx.test(data) && addrRegx.test(data)) {
            ...
            return { address: `获取失败`, ip: finalIp };
        }
    }
    catch (error) {
        return { address: `获取失败`, ip: finalIp };
